...
#-----------------#
#
set --export DOTFILES ~/workspace/dotfiles
...
function killemu 
   ruby ~/workspace/$DOTFILES/ruby/killemu.rb 
end

function gif
  ruby ~/workspace/$DOTFILES/ruby/gif.rb
end

function setup_d
  sh ~/workspace/$DOTFILES/tmux/setup_dotfiles.sh
end

function setup_rn
  sh ~/workspace/$DOTFILES/tmux/setup_rn.sh
end
function awslog 
  sh ~/workspace/$DOTFILES/tmux/login.sh
end

function co
	ruby ~/workspace/$DOTFILES/git/gitbranch.rb 
	end
...
#  gradle clean --info allE2ETests -Dkarate.env=preprod -Dkarate.properties=https://mkazgdah0l.execute-api.us-east-1.amazonaws.com/u0056119 -PtestProfile=a-corporate-preprod -PprojectName=ecpregistry -PtopicARN=a204121-control-ecpmeta-u0056119-use1 -PRegistryQueue=a204121-registryqueue-ecpmeta-u0056119-use1 -Ptestprefix=foo -PSSM_PASSWORD_PARAMETER=a204121-kms-ecpmeta-registry -PBUCKETNAME=a204121-content-ecpmeta-u0056119-use1 -PTABLENAME=a204121-s3indextable-ecpmeta-u0056119-use1 -Dcucumber.options="--tags @Graph"
  echo "fish got args " 
  echo $argv
   ruby ~/workspace/$DOTFILES/ruby/run_if_more_than_60s.rb $argv
end
...
function testargs
  ...
  echo "hello moto"
  echo $argv
  ruby ~/workspace/$DOTFILES/ruby/testargs.rb $argv
end
